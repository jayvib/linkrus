...
import (
	...
	"linkrus/api/linkgraphapi/proto/mocks"
...
func (s *ClientTestSuite) TestLinks(c *gc.C) {
	...
	// Create a matcher for the argument to be received.
	rpcCli.EXPECT().Links(
...
func (s *ClientTestSuite) TestEdges(c *gc.C) {
	...
	// Set a matcher and return value for the rpc client mock
	now := time.Now().Truncate(time.Second).UTC()
	rpcCli.EXPECT().Edges(
		gomock.AssignableToTypeOf(ctxWithCancel),
		&proto.Range{FromUuid: minUUID[:], ToUuid: maxUUID[:], Filter: mustEncodeTimestamp(c, now)},
	...
	// Set matcher and returns for the edge stream
